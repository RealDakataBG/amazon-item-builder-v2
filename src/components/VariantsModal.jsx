              ...
              {steps.map(step => (
                <div key={step.id} className="flex items-start gap-3">
                  <div className="mt-0.5 flex-shrink-0">
                    {step.status === 'pending' && <div className="w-3 h-3 rounded-full bg-gray-200 mt-0.5" />}
                    {step.status === 'running' && <div className="w-3 h-3 rounded-full bg-blue-500 animate-pulse mt-0.5" />}
                    {step.status === 'done'    && <div className="w-3 h-3 rounded-full bg-emerald-500 mt-0.5" />}
                    {step.status === 'error'   && <div className="w-3 h-3 rounded-full bg-red-500 mt-0.5" />}
                  </div>
              ...
              <div className="mt-6 pt-5 border-t border-gray-100 space-y-3">
                ...
                {variantResults.map(r => (
                  <div key={r.label} className="flex items-center gap-3">
                    <span className="text-sm font-semibold text-gray-500 w-8 flex-shrink-0">{r.label}</span>
                    {r.driveUrl ? (
                      <a href={r.driveUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-amber-400 hover:bg-amber-500 text-white transition-colors">
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                        </svg>
                        Drive
                      </a>
                    ) : <span className="text-xs text-gray-300">No Drive URL</span>}
                    {r.sheetUrl ? (
                      <a href={r.sheetUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-emerald-500 hover:bg-emerald-600 text-white transition-colors">
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                        Sheet
                      </a>
                    ) : <span className="text-xs text-gray-300">No Sheet URL</span>}
                  </div>
                ))}
              </div>
            )}
