...
import { fetchIdentifierSheet } from '../utils/sheets'
      ...
      <div className="w-full max-w-2xl mx-4 my-16 bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="px-8 pt-8 pb-6 border-b border-gray-100">
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="text-sm font-semibold text-gray-500">Amazon Listing Builder</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-900 mt-3">Select a Client</h1>
          <p className="text-sm text-gray-500 mt-1">Choose the client you want to create a listing concept for.</p>
        </div>
            ...
            <>
              {filtered.length === 0 ? (
                <div className="text-center py-12 text-gray-400">
                  <p className="text-sm">No clients found{search ? ` for "${search}"` : '.'}</p>
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-3 max-h-96 overflow-y-auto">
                  {filtered.map((client, i) => (
                    <button
                      key={`${client.identifier}-${i}`}
                    ...
                    >
                      ...
                      {client.identifier && (
                        <p className="text-xs text-gray-400 mt-1 font-mono truncate">{client.identifier}</p>
                      )}
                    </button>
                  ))}
                </div>
